...
Layout: A4 Retrato, 2 cards por pÃ¡gina (16cm x 10cm cada), centralizados.
...
def resource_path(relative: str) -> str:
    """Resolve caminho de assets â€” funciona tanto em dev quanto no .exe."""
...
def registrar_fontes():
    """Registra as fontes Agrandir no ReportLab (uma vez por sessÃ£o)."""
    try:
...
# â”€â”€ DimensÃµes do card (em pontos) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# 2 cards por pÃ¡gina: espaÃ§amento igual entre topo, meio e base
_MARGEM_V = (PAGE_H - 2 * CARD_H) / 3   # ~91pt â‰ˆ 3.2cm de espaÃ§o em cada intervalo

# PosiÃ§Ã£o inferior (cy) de cada card â€” ReportLab: y=0 no canto inferior esq.
CARD_CY_TOP = _MARGEM_V + CARD_H + _MARGEM_V  # card de cima
CARD_CY_BOT = _MARGEM_V                         # card de baixo


def _desenhar_card(c: rl_canvas.Canvas, card: dict, logo_path: str,
                   cx: float, cy: float):
    """
    Desenha um card na posiÃ§Ã£o (cx, cy) â€” cy = canto inferior esquerdo do card.
    """
    # --- Borda ---
    c.setLineWidth(2)
    ...
    # LaTeX: posiÃ§Ã£o ~6.2 do topo para formando, ~7 para mesa/homenageado
    ...
    # --- SubtÃ­tulo (curso / cargo / tÃ­tulo) ---
    ...
    try:
        ...
        logo_w = logo_h * ratio
        c.drawImage(logo_path, cx + 0.5 * cm, cy + 0.5 * cm,
...
def _draw_text_centered_wrapped(
    ...
    """Desenha texto centralizado com quebra de linha automÃ¡tica."""
    if not text:
        return
...
def gerar_pdf(dados: DadosCerimonia, caminho_saida: str) -> None:
    """Gera o PDF: A4 Retrato, 2 cards por pÃ¡gina."""
    _garantir_fontes()

    logo_path = resource_path(os.path.join("assets", "logo.png"))

    c = rl_canvas.Canvas(caminho_saida, pagesize=A4)
    c.setTitle(f"Cards Formatura â€” {dados.texto_evento}")
    c.setAuthor("Anthony Perotti")
    c.setCreator("Gerador de Cards PROGRAD/UFSM â€” por Anthony Perotti")
    ...
    # Dois cards por pÃ¡gina: Ã­ndices 0,1 â†’ pÃ¡g 1; 2,3 â†’ pÃ¡g 2; etc.
    for i in range(0, total, 2):
        # Card superior
        card1 = dados.get_card(i)
        _desenhar_card(c, card1, logo_path, CARD_X, CARD_CY_TOP)

        # Card inferior (se existir)
        if i + 1 < total:
            card2 = dados.get_card(i + 1)
            _desenhar_card(c, card2, logo_path, CARD_X, CARD_CY_BOT)
